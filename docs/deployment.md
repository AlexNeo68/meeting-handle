...
## Транскрибация (локальный whisper.cpp)

### Предусловия

- **ffmpeg** — конвертация аудио в WAV (macOS: `brew install ffmpeg`).
- **cmake + Xcode Command Line Tools** — сборка `whisper-cli` из `nodejs-whisper`
  при первом использовании (только на macOS).
- **Интернет** — для автоскачивания модели whisper при первом использовании.

### Как работает

- Модель скачивается автоматически при первом использовании (дефолт `base`,
  ~142 MB) в директорию модели, затем `whisper-cli` собирается через CMake.
- **Warm-up при старте API** (`onModuleInit`) инициирует скачивание модели и
  сборку бинаря в фоне — первая транскрибация не ждёт долгой сборки.
- Транскрибация выполняется **только на CPU** (`-ng`), язык аудио
  автоопределяется. Ограничение параллелизма — `TRANSCRIPTION_CONCURRENCY`
  (in-memory очередь в одном инстансе API).

### Переменные окружения (все из `apps/api/.env.example`)

| Переменная                 | Дефолт  | Назначение                                                                 |
| -------------------------- | ------- | -------------------------------------------------------------------------- |
| `WHISPER_MODEL_NAME`       | `base`  | Модель whisper (см. MODELS_LIST пакета nodejs-whisper).                    |
| `WHISPER_MODEL_DIR`        | —       | Директория модели (опционально; дефолт — `nodejs-whisper/cpp/whisper.cpp/models`). |
| `WHISPER_AUTO_DOWNLOAD`    | `true`  | Автоскачивание модели, если файла нет.                                     |
| `TRANSCRIPTION_CONCURRENCY`| `1`     | Максимум параллельных транскрибаций (int ≥ 1).                             |
| `TRANSCRIPTION_ENABLED`    | `true`  | Полностью отключает транскрибацию: при `false` загрузка файла **не** ставит `PENDING`, а `GET .../transcript` и `POST .../transcription/retry` отвечают `409 'Transcription disabled'`. |

### Поведение при рестарте

Незавершённые транскрибации (`PROCESSING`) при старте API переводятся в
`FAILED` со статусом `Interrupted by server restart` — повторный запуск можно
сделать через `POST .../transcription/retry`.

### Время первой транскрибации

Первая транскрибация включает скачивание модели + сборку `whisper-cli`.
Далее реальное время зависит от длины аудио (базовая модель, CPU).
